...
########################################################
import_sdgs_from_git <- function(sdg, list_with_posteriors = FALSE) {
  ...
  prior_posterior_full_tibble <<-
    ...
    purrr::map(., function(x){
      ...
      RCurl::getURL(stringr::str_c("https://raw.githubusercontent.com/dxiai/sustainability-keywords/main/", type, "/", filename), .encoding = "UTF-8") %>%
...
a <- import_sdgs_from_git(1, FALSE)
# # str(a)
View(a)
...
#* @get /dc_sdg_mapping
  function() {
     ...
     data_mapped <- mapping_data(dataIn = import_data(), sdgIn = import_sdgs_from_git(sdg = 1, list_with_posteriors = FALSE), fconfig = config)
     export_data(data = data_mapped, output = "console")
   }
# View(data_mapped)
