# sustainability-keyword-mapping Reference

SDG identifying keywords for sdg labelling
...
The keywords are organised per SDG. For each SDG there is one Excel-Spreadsheet in the `data/sdgs`-folder. Each Spreadsheet must meet three requirements:

- It is named as `SDG#.xlsx`, where `#` refers to the number of the corresponding SDG. 
- It has one worksheet with the same name as the file (without the file extension).
- The keyword worksheet contains a table with three columns per language:
  - `keyword_LG`
  - `required_context_LG`
  - `forbidden_context_LG`
  
**Important** Using any other column names will lead to system errors for the related files. 
  
`LG` in the column name is a placeholder for the two character language code. The system allows the following languages:
...
It is possible to use other two chacter language-codes. These languages are ignored by the dashboard. However, the indexer will try to on them.

The spreadsheets are organised to keep the related keywords aligned in one row. However, a keyword definition for just one of the languages is valid. 

A indexing term consists of the three fields, of which the `keyword` is **required**. If no `keyword` is provided, the indexer will ignore the other two fields. This will not lead to errors. 

`keywords` are handled as is. I.e., all words provided are required to be present in the provided order to match the record. **Important** The indexer  verifies the exact order of the provided terms, while the index term editor *does not* (so far).

The `required_context` is used to force that the terms listed here MUST be also present in the record to match.

The `forbidden_context` works exactly oposite: the provided term MUST not be present to count for a match. 
...
The matching terms generated by the spreadsheets will generate  tripplets of exactly one term per field. It is not possible to use more complex queries for specifying matching requirements.

## Historical Information

> Everything below this section is due for removal!
